# Contributing to `jul6art/auth-bundle`

`jul6art/auth-bundle` ships the `User` entity of this ecosystem together with its repository
— which doubles as a password upgrader — its manager and its factory. No project in this
ecosystem declares its own `App\Entity\User`; it installs this one and adds validation and
serialization rules from its own configuration.
...
* **Bug** → open an [issue](https://github.com/jul6art/auth-bundle/issues/new/choose)
...
```bash
git clone https://github.com/jul6art/auth-bundle.git
cd auth-bundle
composer install
```
...
`jul6art/auth-bundle` is installed by other projects through Composer, so the version number
...
2. **Password handling goes through Symfony's `PasswordHasherInterface`, always.** No
   hashing algorithm is chosen here, no salt is invented, and the repository's password
   upgrader stays the only place a stored hash is rewritten.

3. **The password and any token belong to no serialization group.** A field that cannot be
   serialized cannot leak through an API, a form or the profiler. Serialization groups for
   this vendor entity live in the application's `config/serializer/`, which is deliberate —
   do not add them to the entity.

4. **Account lookup is exact and case-consistent.** Two accounts differing only in the case
   of an e-mail is an account-takeover vector, not an edge case.

5. **Answers do not reveal whether an account exists.** Anything this bundle returns to an
   unauthenticated caller — a lookup, a password reset, a validation message — must read the
   same for a known and an unknown address.

6. **Adding a method to a contract is a major release.** Applications implement these
   interfaces; see the versioning section above.
